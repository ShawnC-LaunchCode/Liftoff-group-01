...
public class RegistrationController {
    ...
    public ResponseEntity<String> registerUser(@RequestBody RegisterFormDTO registerFormDTO) {
        try {
            // Simple validation
            if (registerFormDTO.getEmail() == null || registerFormDTO.getPassword() == null) {
                return ResponseEntity.badRequest().body("Please provide both email and password for registration.");
            }

            // Check if the user with the given email already exists
            Optional<User> existingUser = userRepository.findByEmail(registerFormDTO.getEmail());
            if (existingUser.isPresent()) {
                return ResponseEntity.badRequest().body("A user with this email already exists.");
            }

            // Create a new user and save to the database
            User newUser = new User(registerFormDTO.getEmail(), registerFormDTO.getPassword(), registerFormDTO.getName());
